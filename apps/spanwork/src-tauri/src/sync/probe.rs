...
const PROBE_DEVICE_PREFIX: &str = "probe-";

/// TCP 发现探测使用的 device_id 前缀；此类连入不触发 UI 通知。
pub fn is_probe_device_id(device_id: &str) -> bool {
    device_id.starts_with(PROBE_DEVICE_PREFIX)
}

/// 短超时 TCP 探测，用于连接失败诊断（不发送 hello）。
pub fn tcp_reachability_label(host: &str, port: u16) -> &'static str {
    let addr = format!("{host}:{port}");
    let socket_addr = match addr.to_socket_addrs() {
        Ok(mut iter) => match iter.next() {
            Some(a) => a,
            None => return "resolve_empty",
        },
        Err(_) => return "resolve_failed",
    };
    match TcpStream::connect_timeout(&socket_addr, Duration::from_millis(PROBE_CONNECT_MS)) {
        Ok(_) => "open",
...
pub fn probe_spanwork_at(
    ...
    let hello = HelloPayload {
        device_id: format!("{PROBE_DEVICE_PREFIX}{local_device_id}"),
    ...
    for chunk in targets.chunks(LAN_PROBE_BATCH) {
        ...
        for target in chunk {
            ...
            handles.push(std::thread::spawn(move || {
                if let Some(peer) = probe_spanwork_at(&target, port, &local_device_id, versions) {
...
pub fn scan_hotspot_peers(
    port: u16,
    local_device_id: &str,
    versions: SyncVersions,
) -> Vec<DiscoveredPeer> {
    let local = local_sync_ipv4_addrs();
    parallel_probe_targets(&hotspot_probe_targets(&local), port, local_device_id, versions)
}

/// 扫描本机 /24 子网上的 Spanwork 监听端口（mDNS 失败时的局域网兜底）。
pub fn scan_lan_peers(
    port: u16,
    local_device_id: &str,
    versions: SyncVersions,
) -> Vec<DiscoveredPeer> {
    let local = local_sync_ipv4_addrs();
    parallel_probe_targets(&lan_subnet_probe_targets(&local), port, local_device_id, versions)
}
