...
describe('formatFrequencyLabel', () => {
  it('shows daily label', () => {
    expect(formatFrequencyLabel({ frequency: 'daily' } as never, t)).toBe(
      t('habit.frequency.daily'),
    );
  });

  it('shows weekly day count', () => {
    expect(formatFrequencyLabel(weeklyRule([1, 3, 5]), t)).toBe(
  ...
  it('shows monthly day count for many selections', () => {
    ...
    expect(formatFrequencyLabel(monthlyRule(days), t)).toBe(
      t('habit.frequency.monthlyCount', { count: 30 }),
    );
  });

  it('shows yearly day count', () => {
    expect(formatFrequencyLabel(yearlyRule(['01-01', '06-01', '12-25']), t)).toBe(
  ...
  it('keeps verbose detail for tooltip', () => {
    ...
    expect(formatFrequencyLabel(rule, t)).toBe(
      t('habit.frequency.monthlyCount', { count: 3 }),
    );
    expect(formatFrequencyLabelVerbose(rule, t, 'zh-CN')).toBe(
